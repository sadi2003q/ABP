"""Train v3 — IMU-anchored rotation model.

Identical CLI to train_v2.py on purpose, so existing v2 commands can
be re-run against v3 by only changing the script name — this keeps
comparisons between v2 and v3 runs apples-to-apples (same flags, same
config dataclass, same defaults).
"""

import argparse, logging, sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent))
from trainer_v3 import TrainConfigV3, TrainerV3

def main():
    p = argparse.ArgumentParser(description="Train v3 (IMU-anchored rotation)")
    ...
    p.add_argument("--save-dir", type=str, default="runs/exp_v3")
