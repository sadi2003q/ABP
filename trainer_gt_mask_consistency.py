...
class TrainerGTMaskConsistency:
    ...
    def _forward_two_pairs(self, batch):
        ...
        # The renderer requires ALL its inputs to share one dtype, and
        # its internal torch.linalg.solve call does not support bf16 at
        # all (CUDA and CPU both lack an LU kernel for it) -- autocast
        # normally protects against this by auto-promoting linalg.solve's
        # inputs back to fp32 inside GTMaskModel.forward()'s own internal
        # renderer calls. But here we call self.renderer() a SECOND time,
        # manually, so we must replicate that fp32 promotion ourselves
        # rather than casting to pred_mask_tm1_probs's (possibly bf16)
        # dtype, which would defeat autocast's protection and reproduce
        # `linalg.solve: ... not implemented for 'BFloat16'`.
        pred_mask_tm1_probs_fp32 = pred_mask_tm1_probs.float()
        depth_tm1 = batch["gt_depth_tm1"].float()
        pose_tm1_to_t = pose_tm1_to_t.float()
        K_for_warp = out_b["K"].float()
        distortion_for_warp = out_b["distortion"].float()
        warped_pred_mask = self.model.renderer(
            feature=pred_mask_tm1_probs_fp32, depth=depth_tm1,
            pose=pose_tm1_to_t, K=K_for_warp, distortion=distortion_for_warp,
        )
