"""
Trainer for GTMaskModel using a SELF-SUPERVISED FEATURE-CONTRASTIVE
loss -- mask prediction from GROUND-TRUTH depth and GROUND-TRUTH pose,
with NO real GT mask used in training (only at evaluation time, to
report IoU/F1 against ground truth for your own judgment).

Difference from the other GTMaskModel trainers
-------------------------------------------------
- trainer_gt_mask.py (TrainerGTMask): DIRECT supervision (BCE+Dice)
  against the real EVIMO2 GT mask. Not self-supervised.
- trainer_gt_mask_photometric.py: self-supervised via BCE against the
  raw warp residual as a per-pixel pseudo-label. Weak/noisy in
  practice (see that file's docstring).
- trainer_gt_mask_consistency.py: self-supervised via agreement
  between the model's own mask predictions at consecutive frame
  pairs. Needs two forward passes per step (memory-heavy).

THIS FILE (TrainerGTMaskContrastive): self-supervised via
FeatureContrastiveLoss (feature_contrastive_loss.py), following the
published unsupervised-motion-segmentation recipe (Yang et al. and
successors): pull together warped/actual FEATURES at confidently
static pixels, push apart at confidently dynamic ones, then read the
mask off the resulting feature-space distance. Only ONE forward pass
per step (same memory profile as the photometric trainer).
"""
...
from src.models.gt_mask_model.feature_contrastive_loss import FeatureContrastiveLoss
...
@dataclass
class TrainConfigGTMaskContrastive:
    ...
    """Fraction of lowest-residual pixels used as confident static anchors."""
...
class TrainerGTMaskContrastive:
    def __init__(self, cfg: TrainConfigGTMaskContrastive):
        self.cfg = cfg
        ...
        self.loss_fn = FeatureContrastiveLoss(
            ...
            for batch_idx, raw_batch in enumerate(self.train_loader):
                ...
                if self.amp_dtype:
                    with torch.amp.autocast('cuda', dtype=self.amp_dtype):
                        ...
                        loss_output = self.loss_fn(outputs["warped_feat"], outputs["feat_t1"], outputs["mask"])
