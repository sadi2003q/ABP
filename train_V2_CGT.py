...
from trainer_V2_CGT import TrainConfigV2, TrainerV2
...
def main():
    ...
    p.add_argument("--use-corrected-gt", action="store_true",
                   help="Use the silhouette-filtered dynamic GT masks "
                        "produced by precompute_corrected_gt.py, instead "
                        "of the original speed-only GT. Requires running "
                        "precompute_corrected_gt.py on every sequence "
                        "first; falls back to original GT with a warning "
                        "for any sequence missing that cache file.")
    args = p.parse_args()
    ...
    cfg = TrainConfigV2(
        ...
        use_corrected_gt=args.use_corrected_gt,
    )
