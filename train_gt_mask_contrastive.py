"""
Train GTMaskModel with a SELF-SUPERVISED FEATURE-CONTRASTIVE loss --
following the published unsupervised-motion-segmentation recipe
(pull static warped/actual feature pairs together, push confidently
dynamic pairs apart, then read the mask off the resulting feature-
space distance), using GROUND-TRUTH depth and GROUND-TRUTH pose for
the warp (no learned geometry). No real GT mask is used in training
-- only at evaluation time, to report IoU/F1.

Usage
-----
python train_gt_mask_contrastive.py \
    ...
    --save-dir runs/exp_gt_mask_contrastive
"""

import argparse, logging, sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent))
from trainer_gt_mask_contrastive import TrainConfigGTMaskContrastive, TrainerGTMaskContrastive


def main():
    p = argparse.ArgumentParser(description="Train GTMaskModel with self-supervised feature-contrastive loss")
    ...
    p.add_argument("--static-percentile", type=float, default=0.3,
                    help="Fraction of lowest-residual pixels used as confident static anchors.")
    p.add_argument("--dynamic-percentile", type=float, default=0.1,
    ...
    cfg = TrainConfigGTMaskContrastive(
    ...
    trainer = TrainerGTMaskContrastive(cfg)
    trainer.train()
