"""
Train GTMaskModel with SELF-SUPERVISED mask-consistency loss.

No GT mask is used in training -- only agreement between the model's
own predictions across two consecutive frame pairs, warped into
alignment using GT depth + GT pose (still ground truth at this stage,
per the incremental plan: prove the consistency loss works before
also swapping in predicted depth/pose). GT mask is used ONLY at eval
time to report IoU/F1 for your own judgment -- never in the loss.

Usage
-----
python train_gt_mask_consistency.py \
    ...
    --save-dir runs/exp_gt_mask_consistency
"""

import argparse, logging, sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent))
from trainer_gt_mask_consistency import TrainConfigGTMaskConsistency, TrainerGTMaskConsistency


def main():
    p = argparse.ArgumentParser(description="Train GTMaskModel with self-supervised mask-consistency loss")
    ...
    p.add_argument("--collapse-penalty-weight", type=float, default=0.5,
                    help="Weight of the anti-collapse regularizer that discourages "
                         "the trivial all-static (empty mask) solution.")
    p.add_argument("--target-dynamic-ratio", type=float, default=0.02,
                    help="Soft floor on predicted dynamic ratio used by the "
                         "anti-collapse regularizer.")
    p.add_argument("--save-dir", type=str, default="runs/exp_gt_mask_consistency")
    ...
    p.add_argument("--balance-dynamic-batches", action="store_true",
                    help="NOTE: for this consistency-loss trainer, batching still "
                         "affects gradient variety, but the loss itself doesn't need "
                         "GT-dynamic frames the way GTMaskLoss did -- still recommended "
                         "so the model sees varied residual magnitudes each batch.")
    ...
    cfg = TrainConfigGTMaskConsistency(
    ...
    trainer = TrainerGTMaskConsistency(cfg)
    trainer.train()
